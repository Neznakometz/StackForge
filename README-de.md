...
- **Prüfe Skills vor der Installation** *(Install-Zeit, jeder Agent)* — bash-guard fängt schlechte Befehle zur *Laufzeit*; die ergänzende Prüfung ist, einen Skill *vor* der Installation zu scannen. Führe [**SkillSpector**](https://github.com/NVIDIA/skillspector) (NVIDIA, Apache-2.0) auf jedem Drittanbieter-Skill — auch unseren — vor der Installation aus: `skillspector scan <repo-or-dir>` erkennt Prompt Injection, Secret-Exfiltration, Supply-Chain und MCP-Poisoning. Referenziert, nicht gebündelt; unsere eigenen Skills werden in CI gescannt (siehe unten).
