...
- **Проверяй скилы до установки** *(install-time, любой агент)* — bash-guard ловит плохие команды в *рантайме*; дополняющая проверка — просканировать скил *до* установки. Прогоняй [**SkillSpector**](https://github.com/NVIDIA/skillspector) (NVIDIA, Apache-2.0) на любом стороннем скиле — включая наши — перед установкой: `skillspector scan <repo-or-dir>` ловит prompt injection, утечку секретов, supply-chain и MCP-poisoning. Упоминается, не бандлится; наши собственные скилы сканируются в CI (см. ниже).
...
- **[Graphify](https://github.com/safishamsi/graphify)** — опциональный **индекс**-граф знаний над проектом на StackForge (строится один раз на Claude, запрашивается из любого агента / MCP) для более дешёвой по токенам навигации в длинном контексте. Рецепты установки по агентам (Claude Code · Cursor · Codex · Antigravity) — в **[`docs/graphify/`](docs/graphify/README.md)**. Упоминается, не бандлится.
